...
class GeminiService:
    ...
    def generate_exam(self, req: GenerateExamRequest) -> ExamPaper:
        ...
        prompt += (
            ...
            "8. MATHEMATICAL FORMATTING: Use standard Markdown LaTeX math delimiters ($...$) strictly for inline math variables, expressions, and equations inside text (e.g. 'where $x = 5$'). Use double dollars ($$...$$) for standalone equations.\n"
            "9. STRICT WORD SPACING RULE: NEVER wrap entire sentences or plain English words in `$` delimiters. Plain English text MUST stay outside LaTeX dollar signs so that word spaces are correctly preserved.\n"
    ...
    def grade_subjective_batch(
        ...
        for g in parsed.graded_questions:
            g.graded_by = "gemini"
        return parsed.graded_questions                "--- CURRICULUM SYLLABUS / SOURCE MATERIAL ---\n"
                f"{req.syllabus_context}\n"
                "----------------------------------------------\n"
                "Align questions directly to learning objectives and concepts in the source material.\n\n"
            )
